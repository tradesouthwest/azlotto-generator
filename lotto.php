 ...
 * @author Larry tradesouthwest.com
...
.button, button {
  ...
  input{height: 1.82em;min-width: 80px; width: auto; max-width: 80px;  font-size: 1.112em;
    letter-spacing: 1px;}
  ...
  .numberz{
    height: 88px;display:block;color: #444;font-weight: bold;background: #fff;padding: 1em; 
...
function getDistinctRandomNumbers ($nb, $min, $max) {
  ...
  <li>Then enter the range of numbers you would like according to which draw game you are playing for.</li>
    ...
    <a class="button right-suide" href="<?php echo
      ...
      <p>
      <?php if( isset( $_POST['submit'] ) && $_POST['action'] == true ) 
      {
        $howmany = (''!= $_POST['howmany'] ) ? $_POST['howmany'] : 6;
        $rangeto = (''!= $_POST['rangeto'] ) ? $_POST['rangeto'] : 44;
        //$gen_random = rand(1, $rangeto);
        $n=range(1, $rangeto);
        shuffle($n);
        $nmbs    = array_slice($n, 0, $howmany );
        for ( $x=0; $x< $howmany; $x++ )
        {
          $nmbs[$x].' ';
        }
        sort($nmbs);
        foreach ( $nmbs as $value ){
